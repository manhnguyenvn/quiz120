...
class Helpers {
     ...
     * @param $keyString The config key path( could be nested in the format main.sub.subsub)
     */
    public static function isConfigEnabled($keyString) {
        $configVal = self::getConfig($keyString);
        return (!empty($configVal) && ($configVal == "true" || $configVal === true));
    }
    /*
     * Get a particular config value
     * @param $keyString The config key path( could be nested in the format main.sub.subsub)
     */
    public static function getConfig($keyString) {
        $config = app('siteConfig');
        $configVal = null;
        try {
            if(!$config)
                throw new Exception('Config empty');
            $configVal = arrayGetValByNestedKeys($config, explode('.', $keyString));
        } catch(Exception $e) {
    ...
    public static function getCacheBusterParam() {
        return 'v=' . Config::get('appMeta')['version'];
    }

    public static function assetWithCacheBuster($path) {
        $url = asset($path);
        ...
        $cacheBusterQueryString = self::getCacheBusterParam();
